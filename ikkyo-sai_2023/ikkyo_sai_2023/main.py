...
async def main_async():
    ...
    frame: tk.Frame = tk.Frame(root)
    frame.pack(side=tk.LEFT, padx=10)
    play_all_button: tk.Button = tk.Button(frame, text="一斉再生", height=10, width=20, command=lambda: [asyncio.create_task(play_async(m, objects[2])) for (m, objects) in zip(moters, buttons)], bg="green", font=("", 20))
    play_all_button.pack()
    stop_all_button: tk.Button = tk.Button(frame, text="一斉停止", height=10, width=20, command=lambda: [asyncio.create_task(stop_async(m, objects[2])) for (m, objects) in zip(moters, buttons)], bg="red", font=("", 20))
